...
function startLocalServer(silent) {
	...
	let s = fork(EXECUTE_SCRIPT, _getLaunchParameters(), { cwd, silent: true });
	...
	s.on('close', (code, signal) => {
		isLocalServerRunningInThisContext = false;
		serverChildProcess = null;
		code && _showError(`local server exit with code ${code}!(Have you launched another local server?)`, {
	...
	function onServerStdout(data) {
		String(data).split('\n').forEach(it => log.d(`[LocalServer/stdout]: ${it}`));
	}
	function onServerStderr(data) {
		String(data).split('\n').forEach(it => log.e(`[LocalServer/stderr]: ${it}`));
	}
}
